...
def beam_search(
    ...
    #if transformer:
    trg_mask = src_mask.new_ones([batch_size*size, 1])  # transformer only
        #trg_mask = src_mask.new_ones([batch_size, 1])
    """ else:
        trg_mask = None """
    ...
    for step in range(max_output_length):
        ...
        #print(encoder_input.size(), decoder_input.size(), trg_embed.size())
        """ print('encoder_input', encoder_input.size())
        print('src_mask', src_mask.size())
        print('trg_embed', trg_embed.size())
        print('trg_mask', trg_mask.size()) """
        logits = decoder( #, hidden, att_scores, att_vectors
            #encoder_output=encoder_output,
            encoder_input=encoder_input,
            #encoder_hidden=encoder_hidden,
            src_mask=src_mask,
            trg_embed=trg_embed,#decoder_input,#trg_embed,
            #hidden=hidden,
        ...
        trg_mask = torch.concat((trg_mask, torch.ones(batch_size*size, 1)), dim=1)
